...
export default function EditService() {
  ...
  const clickSubmit = () => {
    ...
    update(serviceId, userId, jwt.token, serviceData).then((data) => {
      if (data && data.error) {
        setValues({ ...values, error: data.error })
      } else {
    ...
    <div className="max-w-4xl mx-auto px-4 py-12">
      <div className="bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden">
        <div className="p-8 md:p-12 border-b border-gray-50 bg-gray-50/30">
          <h1 className="text-3xl font-extrabold text-gray-900 mb-2">Edit Service</h1>
          <p className="text-gray-500">Update your service details to stay competitive.</p>
        </div>
        ...
        <div className="p-8 md:p-12 space-y-8">
          ...
          <div className="flex flex-col items-center justify-center border-2 border-dashed border-gray-200 rounded-3xl p-10 bg-gray-50 hover:bg-gray-100 transition-colors cursor-pointer relative group">
            ...
            <div className="text-center">
              ...
              <p className="text-gray-900 font-bold mb-1">Click to change service image</p>
              <p className="text-gray-400 text-sm">Recommended: 1600x1200px (4:3)</p>
              {values.image && (
                <p className="mt-4 text-primary font-bold animate-bounce">
                  {typeof values.image === 'string' ? 'Current image retained' : `Selected: ${values.image.name}`}
                </p>
              )}
            </div>
          </div>
